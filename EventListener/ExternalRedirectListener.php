...
class ExternalRedirectListener
{
     ...
     * @param Logger       $logger    A logger, if it's present, detected offenses are logged at the warning level
     * @param UrlGeneratorInterface $generator Router or equivalent that can generate a route, only if override is a route name
     */
    public function __construct($abort = true, $override = null, $forwardAs = null, $targetValidator = null, Logger $logger = null, UrlGeneratorInterface $generator = null)
    {
        if ($override && $abort) {
